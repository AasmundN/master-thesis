...
The delayed weight update scheme is visualized in \autoref{fig:delayed-weight-updates}. For the first samples of each band, weights a
re kept unchanged. From the fifth sample, $t=4$, weights of $t-3$ are used to calculate the weight update. From the sixth sample, a weight refinement step is added. This helps remedy the loss in compression performance by taking the current weights into account when calculating the updated weights. Weight updates using the delayed weight update scheme are summarized as
