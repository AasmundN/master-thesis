\chapter{Results}
\label{sec:results}

\section{Verification Model}

% \begin{table}[ht]
% 	\caption{\acrshort{CPU} time and memory use profile for image compression using C++ predictor and reduced memory usage.}
% 	\label{tab:profile-new}
% 	\begin{center}
% 		\begin{tabular}{lrr}
% 			\hline
% 			\textbf{Module} & \textbf{CPU Time} & \textbf{Memory Use} \\
% 			\hline
% 			Total           & 9.4s              & 438MB               \\
% 			Loading         & 0.9\%             & 17.2\%              \\
% 			Predictor       & 72.6\%            & 13.8\%              \\
% 			Hybrid encoder  & 26.4\%            & 69.0\%              \\
% 			Save outputs    & <0.1\%            & N/A                 \\
% 			\hline
% 		\end{tabular}
% 	\end{center}
% \end{table}

\begin{table}[ht]
	\caption{Performance of compression using verification model with C++ predictor and delayed weight updates averaged over ten HYPSO-2 images for a set of PAEs.}
	\label{tab:coastline-hlm}
	\begin{center}
		\begin{tabular}{lrrrrr}
			\hline
			\textbf{PAE} & \textbf{CR} & \textbf{Loading (s)} & \textbf{Predictor (s)} & \textbf{Encoder (s)} & \textbf{Save Data (s)} \\
			\hline
			0            & 1.995       & 0.311                & 295.273                & 393.706              & 0.069                  \\
			1            & 2.488       & 0.121                & 293.722                & 408.911              & 0.054                  \\
			3            & 3.077       & 0.076                & 294.704                & 471.881              & 0.044                  \\
			7            & 3.912       & 0.076                & 292.520                & 658.729              & 0.034                  \\
			15           & 5.219       & 0.076                & 290.610                & 888.570              & 0.024                  \\
			\hline
		\end{tabular}
	\end{center}
\end{table}

\begin{table}[ht]
	\caption{Performance of compression using verification model with C++ predictor and delayed weight updates for single image tests from HYPSO-2, Aviris and Landsat.}
	\label{tab:performance-results-hlm-compress-cpp}
	\begin{center}
		\begin{tabular}{lrrrrr}
			\hline
			\textbf{Image} & \textbf{Predictor} & \textbf{Samples} & \textbf{Time (s)} & \textbf{Throughput (MS/s)} & \textbf{Memory (MB)} \\
			\hline
			HYPSO-2        & C++                & 78361920         & 821               & 0.095                      & 32415                \\ % & (293)
			Aviris         & C++                & 77643776         & 1166              & 0.066                      & 32091                \\ % & (290)
			Landsat        & C++                & 6291456          & 103               & 0.061                      & 2669                 \\ % & (22)
			\hline
		\end{tabular}
	\end{center}
\end{table}

\begin{table}[ht]
	\caption{Performance of decompression using verification model with C++ predictor and delayed weight updates for single image tests from HYPSO-2, Aviris and Landsat.}
	\label{tab:performance-results-hlm-decompress}
	\begin{center}
		\begin{tabular}{lrrrr}
			\hline
			\textbf{Image} & \textbf{Samples} & \textbf{Time (s)} & \textbf{Throughput (MS/s)} & \textbf{Memory (MB)} \\
			\hline
			Hypso 2        & 78361920         & 1093              & 0.072                      & 32414                \\
			Aviris         & 77643776         & 1834              & 0.042                      & 32091                \\
			Landsat        & 6291456          & 149               & 0.042                      & 2667                 \\
			\hline
		\end{tabular}
	\end{center}
\end{table}

\begin{table}[ht]
	\caption{Performance of compression using verification model with Python predictor and delayed weight updates for single image tests from HYPSO-2 and Landsat.}
	\label{tab:performance-results-hlm-compress-py}
	\begin{center}
		\begin{tabular}{lrrrrr}
			\hline
			\textbf{Image} & \textbf{Predictor} & \textbf{Samples} & \textbf{Time (s)} & \textbf{Throughput (MS/s)} & \textbf{Memory (MB)} \\
			\hline
			HYPSO-2        & Python             & 78361920         & 2818              & 0.027                      & 38996                \\ % & (2169)
			Landsat        & Python             & 6291456          & 263               & 0.024                      & 3204                 \\ % & (161)
			\hline
		\end{tabular}
	\end{center}
\end{table}

\begin{figure}[ht]
	\begin{center}
		\includegraphics[height=0.28\textheight,valign=t]{figures/hypso2-rgb/hypso_decoded.png}
		\includegraphics[height=0.28\textheight,valign=t]{results/aviris/aviris_decoded.png}
		\includegraphics[height=0.28\textheight,valign=t]{results/landsat/landsat_decoded.png}
	\end{center}
	\caption{Decompressed images from HYPSO-2, Aviris and Landsat, compressed with delayed weight update scheme.}
	\label{fig:landsat}
\end{figure}

% \subsection{Debugging}
%
% Clamping of reconstructed sample.
%
% Selection of quantization bin center.
%
% Rounding of signed integers in python vs C++.

\section{Compression Performance}

\begin{table}[ht]
	\caption{Reduction in compression performance averaged over ten HYPSO-2 coastline images for a set of absolute error limits. Results are compared with those of \citeauthor{jiaRemovalFeedbackLoop2025}.}
	\label{tab:coastline-reduction}
	\begin{center}
		\begin{tabular}{lrrrr}
			\hline
			\textbf{PAE} & \textbf{Ordinary} & \textbf{Delayed} & \textbf{Reduction} & \textbf{Reduction, \citeauthor{jiaRemovalFeedbackLoop2025}} \\
			\hline
			0            & 2.002             & 1.995            & $-0.371\%$         & $-1.53\%$                                                   \\
			1            & 2.500             & 2.488            & $-0.470\%$         & $-2.88\%$                                                   \\
			3            & 3.095             & 3.077            & $-0.565\%$         & $-3.86\%$                                                   \\
			7            & 3.941             & 3.912            & $-0.739\%$         & $-4.30\%$                                                   \\
			15           & 5.262             & 5.219            & $-0.826\%$         & $-1.23\%$                                                   \\
			\hline
		\end{tabular}
	\end{center}
\end{table}

% \begin{table}[ht]
% 	\caption{Reductions in compression performance for coastline image for this work and the work by \citeauthor{jiaRemovalFeedbackLoop2025}.}
% 	\label{tab:cr-compared}
% 	\begin{center}
% 		\begin{tabular}{lrr}
% 			\hline
% 			PAE & Coastline, \citeauthor{jiaRemovalFeedbackLoop2025} & Coastline, this work \\
% 			\hline
% 			0   & $-1.53\%$                                          & $-0.371\%$           \\ % 2.03            -> 2.02
% 			1   & $-2.88\%$                                          & $-0.470\%$           \\ % 2.55 (225.86dB) -> 2.53 (225.86dB)
% 			3   & $-3.86\%$                                          & $-0.565\%$           \\ % 3.17 (207.95dB) -> 3.15 (207.94dB)
% 			7   & $-4.30\%$                                          & $-0.739\%$           \\ % 4.07 (192.54dB) -> 4.03 (192.54dB)
% 			15  & $-1.23\%$                                          & $-0.826\%$           \\ % 5.45 (178.10dB) -> 5.39 (178.10dB)
% 			\hline
% 		\end{tabular}
% 	\end{center}
% \end{table}

\begin{table}[ht]
	\caption{Reduction in compression performance for single image tests using relative error limit 16 and absolute error limit 4.}
	\label{tab:performance-results-hlm-dw}
	\begin{center}
		\begin{tabular}{lrrrr}
			\hline
			\textbf{Image} & \textbf{Ordinary} & \textbf{Delayed} & \textbf{Decrease} \\
			\hline
			HYPSO-2        & 2.49              & 2.47             & $-0.80\%$         \\
			Aviris         & 4.41              & 4.31             & $-2.23\%$         \\
			Landsat        & 5.21              & 4.82             & $-7.49\%$         \\
			\hline
		\end{tabular}
	\end{center}
\end{table}

The difference in compression performance shown in \autoref{tab:performance-results-hlm-dw}, especially that of Landsat, are likely due to the characteristics of the specific images used. For example, the Landsat image only has six spectral bands, as opposed to 120 and 200 (?) for Hypso and Aviris.

\begin{table}[ht]
	\caption{PSNR of decompressed images averaged over ten coastline images from HYPSO-2 for a set of PAEs.}
	\label{tab:psnr-compared}
	\begin{center}
		\begin{tabular}{lrr}
			\hline
			\textbf{PAE} & \textbf{CR} & \textbf{PSNR (dB)} \\
			\hline
			0            & 2.02        & N/A                \\ % 2.03            -> 2.02
			1            & 2.53        & 225.86             \\ % 2.55 (225.86dB) -> 2.53 (225.86dB)
			3            & 3.15        & 207.94             \\ % 3.17 (207.95dB) -> 3.15 (207.94dB)
			7            & 4.03        & 192.54             \\ % 4.07 (192.54dB) -> 4.03 (192.54dB)
			15           & 5.39        & 178.10             \\ % 5.45 (178.10dB) -> 5.39 (178.10dB)
			\hline
		\end{tabular}
	\end{center}
\end{table}

Note that the delayed weight update scheme does not affect the PSNR of the reconstructed images. This is due to the maximum error feature of the algorithm. Only the compression performance is affected. Also, PSNR for the ten different images vary only slightly, $\pm0.01dB$. This is also due to max error feature.

% \begin{figure}[ht]
% 	\begin{center}
% 		\includegraphics[width=0.65\textwidth]{figures/relative-psnr-coastline.png}
% 	\end{center}
% 	\caption{}
% 	\label{fig:relative-psnr-coastline}
% \end{figure}

\begin{figure}[ht]
	\begin{center}
		\includegraphics[width=0.6\textwidth]{figures/prediction_bands.png}
	\end{center}
	\caption{Compression performance using relative error limit 16 and absolute error limit 4 for different numbers of prediction bands $P$, with and without delayed weight updates. Results are averaged over a set of ten HYPSO-2 coastline images.}
	\label{fig:prediction_bands}
\end{figure}

\section{RTL Results}

\begin{table}[ht]
	\caption{Resource utilization and maximum clock frequency of the issue 2 hardware accelerator for two different FPGAs, both for the whole accelerator and only the predictor module.}
	\label{tab:performance-results-hw}
	\begin{center}
		\begin{tabular}{lrrrrrr}
			\hline
			\textbf{FPGA}          & \textbf{LUT} & \textbf{FF} & \textbf{DSP} & \textbf{BRAM} & \textbf{Power} & \textbf{Frequency} \\
			\hline
			Zynq-7030              & 5969         & 3463        & 4            & 83.5          & 284 mW         & 92.6 MHz           \\
			Zynq ZU7EV             & 6349         & 3480        & 4            & 79.5          & 890 mW         & 202.0 MHz          \\
			Zynq-7030 (predictor)  & 2866         & 2250        & 4            & 69.5          & 419 mW         & 137.0 MHz          \\
			Zynq ZU7EV (predictor) & 2807         & 2244        & 4            & 69.5          & 1282 mW        & 350.9 MHz          \\
			\hline
		\end{tabular}
	\end{center}
\end{table}
